...
class SphericalLaplaceKernel
{
 ...
 private:
  struct SourceType
  {
    ...
    SourceType() : index(0), point(0) {};
    SourceType(double x, double y, double z) : index(0), point(x,y,z) {};

    explicit operator point_type() const { return point; };
  };
 public:
  ...
  kernel_value_type operator()(const target_type& t,
    ...
    point_type sp = static_cast<point_type>(s);
    point_type tp = static_cast<point_type>(t);
    point_type dist = sp-tp;         //   Vector from target to source
